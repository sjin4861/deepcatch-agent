...
try:
    ...
    logger.warning("python-dotenv가 설치되지 않았습니다. 환경 변수를 직접 설정해주세요.")
...
class FishingRequest(BaseModel):
    """낚시 예약 요청 정보"""
    scenario_type: str = Field(default="long_distance_casting", description="시나리오 타입 (long_distance_casting, boat_fishing)")
    date: str = Field(default="내일", description="희망 날짜 (내일, 이번 주말 등)")
    time: Optional[str] = Field(default="새벽 5시", description="희망 시간 (새벽 5시, 오전 6시 등)")
...
class FishingCallHandler:
    ...
    def generate_initial_prompt(self, state: ConversationState) -> str:
        ...
        return (f"안녕하세요, {state.business_name} 맞으시죠? "
...
async def initiate_fishing_call(call_request: CallRequest):
    ...
    try:
        ...
        if not KO_PHONENUMBER:
            raise HTTPException(status_code=400, detail="KO_PHONENUMBER 환경 변수가 설정되지 않았습니다.")
        
        if not US_PHONENUMBER:
            raise HTTPException(status_code=400, detail="US_PHONENUMBER 환경 변수가 설정되지 않았습니다.")
        
        # 현재 진행 중인 통화가 있는지 확인
        active_calls = [state for state in conversation_states.values() 
        ...
        logger.info(f"전화 연결 시작: {call_request.business_name} (from: {US_PHONENUMBER} -> to: {KO_PHONENUMBER})")
...
async def process_speech_result(
    ...
    answering_machine_phrases = [
        "메시지", "녹음", "신호음", "삐", "번을 눌러", "전화번호를 남기", 
        "사용 방법", "시간이 지났습니다", "나중에 다시", "휴무", "영업시간"
    ]
    ...
    if state.step == ConversationStep.GREETING:
        ...
        if state.fishing_request.scenario_type == "boat_fishing":
            # 선상낚시 시나리오
            if "가능" in speech_lower and ("몇" in speech_lower and ("명" in speech_lower or "분" in speech_lower)):
                # "네 가능합니다. 몇 분이세요?"
            ...
            elif "몇" in speech_lower and ("명" in speech_lower or "분" in speech_lower):
                # 바로 "몇 분이세요?" 물어봄
                state.step = ConversationStep.AWAITING_ALTERNATIVE_DATE
                gather = response.gather(
                    input='speech',
        ...
        if "안 되" in speech_lower or "없어서" in speech_lower or "10월" in speech_lower:
        ...
        if "있습니다" in speech_lower or "가능" in speech_lower or "오후" in speech_lower:
...
async def get_call_result(call_sid: str):
    ...
    return {
        ...
        "failure_reason": state.extracted_info.get('failure_reason', None) if not state.is_successful else None
    }

# 지침에 따른 API 엔드포인트 추가

@app.post("/chat")
async def chat_message(message: str, db: Session = Depends(get_db)):
    """사용자 메시지 받기 (지침 요구사항)"""
    try:
        # 사용자 정보 수집 로직 (간단한 예시)
        plan = crud.get_plan(db)
        
        # 메시지에서 정보 추출 (실제로는 더 정교한 NLP 필요)
        updates = {}
        if "날짜" in message or "일" in message:
            # 간단한 날짜 추출 로직
            if "내일" in message:
                updates["date"] = "내일"
            elif "오늘" in message:
        ...
        if missing:
            response = f"다음 정보가 더 필요합니다: {', '.join(missing)}"
        else:
            response = "모든 정보가 수집되었습니다. 낚시집을 찾아보겠습니다."
            
        return {"response": response, "message": message, "missing_fields": missing}
        
    except Exception as e:
        logger.error(f"채팅 처리 오류: {str(e)}")
        return {"response": "죄송합니다. 처리 중 오류가 발생했습니다.", "error": str(e)}
...
async def save_reservation(reservation: Reservation, db: Session = Depends(get_db)):
    ...
    try:
        ...
        logger.info(f"예약 결과 저장: {reservation.business_name} - {reservation.success}")
