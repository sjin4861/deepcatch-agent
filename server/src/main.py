...
class CallRequest(BaseModel):
    """전화 요청"""
    phone_number: str = Field(default=US_PHONENUMBER, description="전화번호")
    business_name: str = Field(default=KO_PHONENUMBER, description="업체명")
...
async def initiate_fishing_call(call_request: CallRequest):
    ...
    try:
        ...
        call = client.calls.create(
            to=call_request.business_name,
            from_=call_request.phone_number,
            url=webhook_url,
            method='POST',
